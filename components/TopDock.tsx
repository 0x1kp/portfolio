...
export function TopDock({ showThemeToggle = false, links }: Props) {
  const pathname = usePathname();

  const resolved = useMemo(
    () => ({
      github: links?.github ?? "https://example.com/github",
      linkedin: links?.linkedin ?? "https://example.com/linkedin",
      x: links?.x ?? "https://example.com/x",
    }),
    [links]
  );
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-0 md:top-4 z-30 mx-auto mb-4 flex origin-bottom h-full max-h-14">
      ...
      <div className="w-max p-2 rounded-full border z-50 pointer-events-auto relative mx-auto flex min-h-full h-full items-center px-1 dark:bg-gray-900 bg-background [box-shadow:0_0_0_1px_rgba(0,0,0,.03),0_2px_4px_rgba(0,0,0,.05),0_12px_24px_rgba(0,0,0,.05)] transform-gpu dark:[border:1px_solid_rgba(255,255,255,.1)] dark:[box-shadow:0_-20px_80px_-20px_#ffffff1f_inset]">
        ...
        {/* divider */}
        <div data-orientation="vertical" role="none" className="shrink-0 bg-border w-[1px] h-full" />
        ...
        <div className="flex aspect-square cursor-pointer items-center justify-center rounded-full" style={{ width: 40 }}>
          <a className="inline-flex items-center justify-center whitespace-nowrap text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 hover:bg-accent hover:text-accent-foreground rounded-full size-12" aria-label="X" data-state="closed" href={resolved.x} target="_blank" rel="noopener noreferrer">
            <svg viewBox="0 0 24 24" className="size-4" aria-hidden="true"><path fill="currentColor" d="M18.901 1.153h3.68l-8.04 9.19L24 22.846h-7.406l-5.8-7.584-6.638 7.584H.474l8.6-9.83L0 1.154h7.594l5.243 6.932ZM17.61 20.644h2.039L6.486 3.24H4.298Z"/></svg>
          </a>
        </div>

        {/* Theme toggle (off by default) */}
        {showThemeToggle ? (
          <div className="flex aspect-square cursor-pointer items-center justify-center rounded-full" style={{ width: 40 }}>
            <button
